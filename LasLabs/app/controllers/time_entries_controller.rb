class TimeEntriesController < ApplicationController
  ...
  def index
    if current_user.admin?
      @time_entry = TimeEntry.where(invoice_id: nil).order(:start)
    else
      @time_entry = TimeEntry.where(invoice_id: nil).where(company_id: current_user.company_id).order(:start)
    end
  end
  ...
  private
    ...
    def time_entry_params
      params.require(:time_entry).permit(:start, :end, :rate, :description, :tags, :invoice, :user)
    end
